...
/**
 * UserMenu — avatar + hover card with proper server-side logout.
 *
 * Fix: handleLogout now calls POST /auth/logout to invalidate the
 * refresh-token cookie on the server before clearing localStorage.
 * Without this, the refresh token stayed valid server-side after logout.
 */
class UserMenuComponent extends Component<{ navigate: (path: string) => void }, UserMenuState> {
  ...
  handleLogout = async () => {
    ...
    try {
      // Invalidate the refresh-token cookie on the server first.
      await api.post('/auth/logout');
    } catch {
      // Proceed with client-side logout even if the server call fails
      // (e.g. token already expired). This is intentional — always clear local state.
    } finally {
  ...
  render() {
    const { isHovered, userName, userEmail, isLoggingOut } = this.state;
            ...
            <div className="bg-[#111] border border-[#222] rounded-xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] p-5">
              ...
              <div className="space-y-1">
                ...
                <button
                  ...
                  className="w-full text-left px-3 py-2 rounded-lg text-[13px] font-medium text-red-500 hover:bg-red-500/10 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
