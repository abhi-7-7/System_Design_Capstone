...
class ProfileComponent extends Component<ProfileProps, ProfileState> {
  ...
  async componentDidMount() {
    try {
      const data = await this.userApiService.getProfile();
      // The /user/me endpoint returns userId; name/email come from localStorage
      // (set at login time). We use the API call to verify the session is alive.
      this.setState({
        userName: localStorage.getItem('userName') || this.state.userName,
        userEmail: localStorage.getItem('userEmail') || this.state.userEmail,
        isLoading: false,
        error: null,
      });
      // Suppress unused-variable warning — we use `data` to confirm session.
      void data;
    } catch (err) {
      this.setState({
  ...
  handleBack = () => this.props.navigate('/dashboard');
  handleToggleEdit = () => this.setState((prev) => ({ isEditing: !prev.isEditing, error: null }));

  handleInputChange = (
    e: React.ChangeEvent<HTMLInputElement>,
    field: 'userName' | 'userEmail'
  ) => {
    this.setState({ [field]: e.target.value } as Pick<ProfileState, 'userName' | 'userEmail'>);
  };
  ...
  getInitials(name: string): string {
    if (!name || name === 'Smart User') return 'SF';
  ...
  render() {
    const { userName, userEmail, isEditing, isLoading, isSaving, error } = this.state;
        ...
        <main className="flex-1 flex items-center justify-center p-6">
          {isLoading ? (
            <div className="flex flex-col items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-indigo-600 to-purple-500 flex items-center justify-center text-white font-black animate-pulse">SF</div>
            ...
            <div className="w-full max-w-lg bg-[#111] border border-[#222] rounded-2xl shadow-[0_0_50px_rgba(0,0,0,0.5)] p-10 relative overflow-hidden animate-scale-up">
              ...
              <div className="text-center mb-10">
                ...
                <p className="text-indigo-400 font-medium tracking-wide text-sm">SmartFlow Member</p>
              </div>
                ...
                <div className="flex gap-3">
                  {isEditing ? (
                    <>
                      ...
                      <button onClick={this.handleSave} disabled={isSaving} className="px-5 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-semibold shadow-lg shadow-indigo-500/20 transition-all disabled:opacity-50">
